...
function pendingList(){
	function filterReports(status) {
		...
		rows.forEach(function(row) {
			if (status === 'all' || row.cells[2].innerText.toLowerCase().includes(status)) {
				row.style.display = '';
			} else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const allBtn = document.getElementById('all-Complaints');
    ...
    function filterReports(status) {
        allRows.forEach(row => {
            const rowStatus = row.querySelector('.status').classList.contains('completed') ? 'completed' : 'pending';
